...
Deno.serve(async () => {
  ...
  for (const schedule of due ?? []) {
    try {
      if (resendKey) {
        const res = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: String(schedule.recipients).split(",").map((r) => r.trim()).filter(Boolean),
      ...
      await supabase
        .from("report_schedules")
        .update({ last_sent_at: now, next_run_at: nextRun(String(schedule.frequency), new Date()) })
        .eq("id", schedule.id)
      results.push(`delivered ${schedule.id}`)
    } catch (err) {
      results.push(`failed ${schedule.id}: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
